...
- 手动模式：百分比风速滑块。
- 自动温控：CPU、WiFi、CPU/WiFi/主板最高温或平均温、EMC2104 主板温度。
- 普通设置：起转温度、停转温度、满速温度。
- 高级设置：低温风速、CPU 核心选择、自定义四点曲线。
- 原生 LuCI 配置表单及“保存并应用 / 保存 / 重置”按钮。
- 实时显示 CPU 最高温度、WiFi 温度、主板温度、控制服务状态与已应用的 PWM 输出。

主板温度取 EMC2104 三路测温中的最高值，与主板温控的取值规则一致。
CPU/WiFi/主板组合温度源会将 CPU 核心、WiFi 和 EMC2104 三路温度一起参与最高值或平均值计算。
